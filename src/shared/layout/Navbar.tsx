...
const Navbar = () => {
  ...
  return (
    //UI//
    <nav className={`fixed  w-full bg-transfer border-b z-50 
    ${isScrolled ? "bg-linear-to-b from-aman-dark via-aman-black to-aman-dark shadow-lg" : "bg-transparent "}`}>
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        ...
        >
          {isOpen ? <HiX size={28} /> : <HiMenuAlt3 size={28} />} {/*when it is open true = apear x else humburgur icon */}
        </button>
      </div>
      ...
      >
        {links.map((link) => (
          <Link
            ...
            className={ 
              pathname === link.path ? "text-aman-blue font-bold font-inter" : "text-aman-dark font-semibold"
