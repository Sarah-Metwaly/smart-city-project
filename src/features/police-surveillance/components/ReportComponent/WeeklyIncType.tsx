...
function CustomMark(props: any) {
  ...
  return (
    <g>
      <circle cx={x} cy={y} r={4} fill={color || '#E05A5A'} {...other} />
    </g>
  );
}
...
export default function WeeklyIncType() {
  ...
  const dummyLabels = ['SAT', 'SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI'];
  ...
  if (isError || !data?.length)
    return (
      ...
      >
        <span style={{ color: '#E05A5A', fontSize: 12 }}>
          Failed to load data
        </span>
      </Box>
    );
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(90deg, rgba(88,113,125,0.25), transparent)',
          position: 'relative',
          zIndex: 1,
        }}
      />
