...
export default function CrimeStatus() {
  const { Incidents, isError, isLoading } = useIncidents(
    '/api/v1/incidents/DailyIncidents',
  );
  const highCount =
    Incidents?.filter((i) => i.priority?.toUpperCase() === 'HIGH').length ?? 0;
  const mediumCount =
    Incidents?.filter((i) => i.priority?.toUpperCase() === 'MEDIUM').length ??
    0;
  const lowCount =
    Incidents?.filter((i) => i.priority?.toUpperCase() === 'LOW').length ?? 0;
  ...
  const data = [
    ...
    },
    {
      ...
      label: 'Medium',
      color: '#ffb338',
      pct: total ? Math.round((mediumCount / total) * 100) : 0,
    },
    {
      id: 2,
      value: lowCount,
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col justify-between flex-1 h-full min-h-0 p-4 pb-3">
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            >
              <span className="text-[9px] font-mono tracking-[0.15em] text-[#58717D]/55 uppercase">
                Total
              </span>
              <span
        ...
        {isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="relative w-8 h-8">
                <div
                  ...
                  style={{ border: '1px solid rgba(88,113,125,0.15)' }}
                />
                <div className="absolute inset-0 rounded-full border-2 border-t-[#58717D] border-transparent animate-spin" />
              </div>
              <span className="text-[9px] font-mono tracking-[0.2em] text-[#58717D]/50 uppercase">
                Loading
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center flex-1">
            <div className="flex flex-col items-center gap-2">
              <div
                className="flex items-center justify-center w-8 h-8 text-sm font-bold text-red-400 rounded-full"
                style={{
              ...
              <span className="text-[9px] font-mono tracking-[0.2em] text-red-400/60 uppercase">
                Error loading data
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-between flex-1 min-h-0">
            <div className="flex items-center justify-center w-full my-auto">
              <DonutChart data={data} width={200} height={180} />
            </div>
            ...
            <div className="flex flex-col gap-1.5 mt-auto">
              {data.map((item) => (
                <div key={item.id} className="flex items-center gap-2.5">
                  ...
                  />
                  <span
                    className="text-[10px] font-mono tracking-[0.12em] uppercase w-12 flex-shrink-0"
                  ...
                  <div
                    ...
                    style={{ background: 'rgba(88,113,125,0.15)' }}
                  >
                    <div
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <span
                      ...
                      style={{ color: 'rgba(88,113,125,0.45)' }}
                    >
                      {item.pct}%
                    </span>
                  </div>
