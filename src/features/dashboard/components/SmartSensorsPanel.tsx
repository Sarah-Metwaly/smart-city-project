...
const SmartSensorPanel = () => {
  const { LDRData, DHT11Value, MQResponse, getPercent, TotalPower, isLoading, isError } =
    useActivePower();
  const { flameSensorData } = useFlameSensor();

  const sensors = [
    { id: 0, label: "LDR", realValue: LDRData?.power ?? 0, value: getPercent(LDRData?.power ?? 0), color: "#14B8A6" },
    { id: 1, label: "DHT11", realValue: DHT11Value?.power ?? 0, value: getPercent(DHT11Value?.power ?? 0), color: "#3B82F6" },
    { id: 2, label: "MQ", realValue: MQResponse?.power ?? 0, value: getPercent(MQResponse?.power ?? 0), color: "#F59E0B" },
    { id: 3, label: "Flame", realValue: flameSensorData?.power ?? 0, value: getPercent(flameSensorData?.power ?? 0), color: "#FACC15" },
  ];
  ...
  return (
    <div className="relative h-full flex flex-col rounded-xl border border-aman-teal bg-aman-dark px-3 sm:px-4 py-3 sm:py-3.5 overflow-hidden">
      ...
      {isError && <p className="text-[12px] text-red-500">Failed to load sensor data.</p>}

      {!isLoading && !isError && (
        <>
          {/* Stats row */}
          <div className="grid grid-cols-3 gap-2 mb-3 shrink-0">
            <Stat label="TOTAL" value={`${TotalPower}W`} className="text-aman-white" />
            <Stat label="TOP LOAD" value={highest.label} className="text-amber-500" />
            <Stat label="SHARE" value={`${highest.value}%`} className="text-emerald-400" />
          </div>
